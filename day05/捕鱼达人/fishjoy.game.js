(function() {
	...
	c.convertPlistImage = function() {
		...
		this.translateImage(this.fish, a, a[0].sourceSize[0], a[0].sourceSize[1],
			!0, b);
		a = [{
	...
	c.cacheImage = function(a, b, e, d, c) {
		...
		return c ? (a = new Image, a.src = i.toDataURL("image/png"), a.width = i.width, a.height = i.height, document.body.appendChild(
			a), {
			image: a,
			frames: l
		}) : {
...
(function() {
	...
	a.startup = function() {
		...
		this.loader = null;
		if (Q.isWebKit && !Q.supportTouch) document.body.style.webkitTouchCallout = "none", document.body.style.webkitUserSelect =
			"none", document.body.style.webkitTextSizeAdjust = "none", document.body.style.webkitTapHighlightColor =
			"rgba(0,0,0,0)";
	...
	b.prototype.update = function() {
		for (var e = 0; e < this.fishes.length; e++) {
			...
			if (b.captured) this.fishes.splice(e, 1), e--;
				...
				if (b.hasShown || b.changeDirCounter < -a.fps * 10) this.fishes.splice(e, 1), this.fishPool.push(b), b.parent.removeChild(
					b), e--
			} else if (!b.hasShown) b.hasShown = !0
		}
		if (--this.makeCounter <= 0) this.makeCounter = this.fishes.length < this.minNumFishScreen ? a.fps * 2 : a.fps * 3,
			this.makeFish()
	};
...
(function() {
	(Q.use("fish").Utils = {}).calcDirection = function(c, a) {
		var b;
		c.x == a.x ? (b = a.y > c.y ? Math.PI / 2 : -Math.PI / 2, b *= Q.RAD_TO_DEG) : (b = Math.atan(1 / ((a.y - c.y) / (a
			.x - c.x))), b *= Q.RAD_TO_DEG, a.y < c.y ? a.x < c.x && (b += 180) : b = 90 - b);
		return {
			degree: b,
...
(function() {
	...
	b.prototype.changeDirection =
		function(b) {
			...
			else if (Math.random() > 0.8) b = Math.random() > 0.5 ? 1 : -1, this._destRotation = this.rotation + (Math.random() *
				10 + 20 >> 0) * b >> 0;
			var b = a.fps,
				c = b * 5;
	...
	b.prototype.update = function() {
		...
		} else this.moving && (this.x += this.speedX, this.y += this.speedY), this._destRotation != null ? (b = this._destRotation -
				this.rotation, d = b > 0 ? 0.1 : -0.1, h = this.rotation + d, b == 0 || d > 0 && h >= this._destRotation || d < 0 &&
				h <= this._destRotation ? (this.setDirection(this._destRotation), this._destRotation = null) : this.setDirection(
					h)) :
			--this.changeDirCounter <= 0 && this.canTurning && this.changeDirection()
	};
	b.prototype.isOutOfScreen = function() {
		if (this.x < -this.width || this.x > a.width + this.width || this.y < -this.height || this.y > a.height + this.height)
			return !0;
...
(function() {
	...
	b.prototype.update = function() {
		this.isOutOfScreen() ? this.destory() : (this.x += this.speedX, this.y -= this.speedY, this.checkCollision() &&
			this.destory())
	};
	b.prototype.checkCollision = function() {
		for (var b = a.fishManager.fishes, d = b.length, h = !1, i = 0; i < d; i++) {
			...
			if (!(this.y - this.height * 0.5 > g.y + g.height * 0.5 ||
					this.y + this.height * 0.5 < g.y - g.height * 0.5 || this.x - this.width * 0.5 > g.x + g.width * 0.5 || this.x +
					this.width * 0.5 < g.x - g.width * 0.5) && this.hitTestObject(g, !0)) {
				h = !0;
				break
			}
		}
		...
		for (i = 0; i < d; i++)
			if (g = b[i], !(f.y - f.height * 0.5 > g.y + g.height *
					0.5 || f.y + f.height * 0.5 < g.y - g.height * 0.5 || f.x - f.width * 0.5 > g.x + g.width * 0.5 || f.x + f.width *
					0.5 < g.x - g.width * 0.5) && f.hitTestObject(g, !0) && g.canBeCaptured(this.power - 1)) g.moving = !1, g.captured = !
				0, g.capturingCounter = a.fps >> 1, g.gotoAndPlay("capture");
		return !0
	};
