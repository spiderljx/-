(function(b) {
	...
	c.createDOMDrawable = function(a, b) {
		...
		k.style.width = i + "px";
		k.style.height = j + "px";
		if (d == "canvas") k.width = i, k.height = j, g && (d = k.getContext("2d"), i = b.rect || [0, 0, i, j], d.drawImage(
			g,
			i[0], i[1], i[2], i[3], a.x || 0, a.y || 0, a.width || i[2], a.height || i[3]));
		else if (k.style.opacity = a.alpha != void 0 ? a.alpha : 1, k.style.overflow = "hidden", g && g.src) k.style.backgroundImage =
			"url(" + g.src + ")", k.style.backgroundPosition = -(a.rectX || 0) + "px " + -(a.rectY || 0) + "px";
		return k
	};
	...
	c.hitTestObject = function(a, b, d) {
		a = a.getBounds();
		b = b.getBounds();
		return (a = a.x <= b.x + b.width && b.x <= a.x + a.width && a.y <= b.y + b.height && b.y <= a.y + a.height) && d ?
			(a = c.polygonCollision(b, b), a !== !1) : a
	};
...
(function() {
	...
	b.prototype._onEvent = function(a, c, d) {
		var e = a,
			f = a.type;
		if (a.type.indexOf("touch") == 0) e = a.touches && a.touches.length > 0 ? a.touches[0] : a.changedTouches && a.changedTouches
			.length > 0 ? a.changedTouches[0] : a, e.type = f;
...
(function() {
	...
	Quark.getUrlParams = function() {
		...
		var a = {},
			...
			b = c.indexOf("?");
		if (b > 0)
			for (var c = c.substring(b + 1).split("&"), b = 0, d; d = c[b]; b++) e = c[b] = d.split("="), d = e, a[d[0]] = d.length >
				1 ? d[1] : !0;
		return a
	};
	var a = document.head,
...
(function() {
	...
	b.prototype._run = function() {
		if (!this.paused) {
			...
			for (var b = 0, e = this._listeners.length, f, h; b < e; b++) f = this._listeners[b], h = f.__runTime || 0, h == 0 ?
				f.step(this.info) : c > h && (f.step(this.info), this._listeners.splice(b, 1), b--, e--);
			a.lastTime = c
		}
	};
...
(function() {
	var b = Quark.Tween = function(c, b, e) {
		...
		for (var f in b) {
			var h = c[f],
				g = b[f];
			h !== void 0 && typeof h == "number" && typeof g == "number" && (this._oldProps[f] = h, this._newProps[f] = g,
				this._deltaProps[f] =
				g - h)
		}
			...
			if (!(d < 0)) {
				...
				if (a >= 1) {
					if (this.reverse) {
						...
						this._frameCount = 0
					} else if (b.remove(this), e = this.next, e != null && (e instanceof b ? (a = e, e = null) : a = e.shift(), a !=
							null)) a.next = e, a.start();
	...
	a.Exponential.EaseInOut = function(a) {
		return a == 0 ? 0 : a == 1 ? 1 : (a *= 2) < 1 ? 0.5 * Math.pow(2, 10 * (a - 1)) : 0.5 * (-Math.pow(2, -10 * (a - 1)) +
			2)
	};
	...
	a.Elastic.EaseInOut = function(a) {
		...
		return (a *= 2) < 1 ? -0.5 * e * Math.pow(2,
			10 * (a -= 1)) * Math.sin((a - b) * 2 * Math.PI / f) : e * Math.pow(2, -10 * (a -= 1)) * Math.sin((a - b) * 2 *
			Math.PI / f) * 0.5 + 1
	};
	...
	a.Back.EaseInOut = function(a) {
		return (a *= 2) < 1 ? 0.5 * a * a * (3.5949095 * a - 2.5949095) : 0.5 * ((a -= 2) * a * (3.5949095 * a + 2.5949095) +
			2)
	};
...
(function() {
	...
	b.prototype._evtHandler = function(a) {
		if (a.type == "progress") {
			...
			if (d / a.target.duration >= 1) this._element.removeEventListener("progress", this._evtHandler), this._element.removeEventListener(
				"error", this._evtHandler), this._loaded = !0, this.dispatchEvent({
				type: "loaded",
				target: this
...
(function() {
	...
	b.prototype.set = function(a, b) {
		if (a instanceof HTMLImageElement || a instanceof HTMLCanvasElement || a instanceof HTMLVideoElement) this.rawDrawable =
			a;
...
(function() {
	var b = Quark.DisplayObject = function(a) {
		...
		this._depth = 0;
		this._lastState = {};
		this._stateList = ["x", "y", "regX", "regY", "width", "height", "alpha", "scaleX", "scaleY", "rotation", "visible",
			"_depth"
		];
	...
	b.prototype.setDrawable = function(a) {
		this.drawable == null ? this.drawable = new Quark.Drawable(a) : this.drawable.rawDrawable != a && this.drawable.set(
			a)
	};
...
(function() {
	...
	b.prototype._onEvent = function(a) {
		...
		a.eventX = b;
		a.eventY = d;
		if (e != null && e != f) a.lastEventTarget = e, (b = a.type == "mousemove" ? "mouseout" : a.type == "touchmove" ?
...
(function() {
	...
	b.prototype.nextFrame = function(a) {
		...
		return b.interval > 0 && this._displayedCount > 0 ? this.currentFrame : this.currentFrame >= this._frames.length -
			1 ? this.currentFrame = 0 : ++this.currentFrame
	};
	b.prototype._update = function(a) {
		var c = this._frames[this.currentFrame];
		c.stop ? this.stop() : (this.paused || this.nextFrame(this.useFrames ? 1 : a && a.deltaTime), this.setRect(c.rect),
...
(function() {
	...
	b.prototype.setEnabled = function(a) {
		...
		(this.eventEnabled = this.enabled = a) ? this.currentFrame == 3 && this._skin.gotoAndStop(b.UP): this.disabledState ?
			this._skin.gotoAndStop(b.DISABLED) : this._skin.gotoAndStop(b.state.UP);
		return this
	};
...
(function() {
	function b(a, b) {
		var c = "";
		c += b ? "translate3d(" + (a.x - a.regX) + "px, " + (a.y - a.regY) + "px, 0px)rotate3d(0, 0, 1, " + a.rotation +
			"deg)scale3d(" + a.scaleX + ", " + a.scaleY + ", 1)" : "translate(" + (a.x - a.regX) + "px, " + (a.y - a.regY) +
			"px)rotate(" + a.rotation + "deg)scale(" + a.scaleX + ", " + a.scaleY + ")";
		return c
	}
	...
	h.prototype.draw = function(a) {
		if (!a._addedToDOM) {
			...
			b == null && c.parentNode == null ? this.canvas.appendChild(c) : (b = b.getDrawable(this), c.parentNode != b && b.appendChild(
				c));
			a._addedToDOM = !0
		}
	};
	h.prototype.transform = function(a) {
		...
		if (a.transformEnabled || !a._addedToDOM) {
			...
			if (!c.style.width || a.propChanged("width", "height")) c.style.width = a.width + "px", c.style.height = a.height +
				"px";
			if (!c.style[e] || a.propChanged("regX", "regY")) c.style[e] = a.regX + "px " + a.regY + "px";
			if (!c.style[d] || a.propChanged("x", "y", "regX", "regY", "scaleX", "scaleY", "rotation")) e = Quark.supportTransform3D ?
				b(a, !0) : b(a, !1), c.style[d] = e;
